...
public class PauseMenuScreen implements Screen {
    ...
    private void createPauseMenu() {
        ...
        // Create a checkbox for enabling/disabling debugging
        Texture checkboxOn = new Texture(Gdx.files.internal("UI/GrayButtons/Music On.png"));
        Texture checkboxOff = new Texture(Gdx.files.internal("UI/GrayButtons/Music Off.png"));
        ...
        checkBoxStyle.font = new BitmapFont(); // Use default font

        CheckBox debugCheckBox = new CheckBox(" Debug Mode ", checkBoxStyle);
        debugCheckBox.setChecked(gameLoop.isDebugging);
        debugCheckBox.addListener(new ClickListener() {
            @Override
            public void clicked(InputEvent event, float x, float y) {
                gameLoop.isDebugging =(debugCheckBox.isChecked());
            }
        });
        ...
        table.row(); // Move to next row for the checkbox
        table.add(debugCheckBox).colspan(3).center().pad(10);
